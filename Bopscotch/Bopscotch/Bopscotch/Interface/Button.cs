...
namespace Bopscotch.Interface
{
    public class Button
    {
        ...
        public ButtonIcon Icon { private get; set; }
        ...
        {
            spriteBatch.Draw(TextureManager.Textures[_highlightTextureName], ParentPosition + _highlightPosition, null, TintFromActivationState,
                0.0f, Vector2.Zero, _scale, SpriteEffects.None, Highlight_Render_Depth);

            spriteBatch.Draw(TextureManager.Textures[Icon_Container_Texture], ParentPosition + _topLeft, null,
                TintFromEnabledState(IconBackgroundTint), 0.0f, Vector2.Zero, _scale, SpriteEffects.None, Icon_Container_Render_Depth);

            if (Icon != Button.ButtonIcon.None)
            {
                spriteBatch.Draw(TextureManager.Textures[Icon_Texture], ParentPosition + _iconPosition, IconFrame(Icon), Color.Black, 0.0f,
                    Vector2.Zero, _scale, SpriteEffects.None, Icon_Render_Depth);
            }

            if (!_hideCaption)
            {
                spriteBatch.Draw(TextureManager.Textures[Text_Container_Texture], ParentPosition + _textContainerPosition, null,
                    TintFromEnabledState(Color.White), 0.0f, Vector2.Zero, _scale, SpriteEffects.None, Text_Container_Render_Depth);

                TextWriter.Write(Translator.Translation(_caption), spriteBatch, ParentPosition + _textPosition, TintFromEnabledState(Color.Black), 
        ...
        public static Rectangle IconFrame(ButtonIcon buttonIcon)
        {
            return new Rectangle((int)buttonIcon * Definitions.Button_Icon_Pixel_Size, 0,
        ...
        private const float Text_Vertical_Offset = -10.0f;
