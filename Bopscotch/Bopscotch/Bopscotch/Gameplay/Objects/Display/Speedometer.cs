...
namespace Bopscotch.Gameplay.Objects.Display
{
    public class Speedometer : ISimpleRenderable, IAnimated, ITransformationAnimatable
    {
        ...
        public Vector2 Origin { get { return _masterOrigin; } }
        ...
        {
            _masterOrigin = new Vector2(TextureManager.Textures[Base_Texture_Name].Width, TextureManager.Textures[Base_Texture_Name].Height) / 2.0f;
            ...
            _centerOffset = new Vector2(-_masterOrigin.X, _masterOrigin.Y);
        }
        ...
        {
            SetActiveSegments();
            spriteBatch.Draw(TextureManager.Textures[Base_Texture_Name], CenterPosition + _centerOffset, null, Color.White, 0.0f, _masterOrigin, 1.0f, 
                SpriteEffects.None, Base_Render_Depth);

            for (int i = 0; i < _segmentTint.Length; i++)
            {
                spriteBatch.Draw(TextureManager.Textures[Segment_Texture_Name], CenterPosition + _centerOffset, null, (_segmentIsActive[i] ? _segmentTint[i] : Color.Gray),
                    MathHelper.ToRadians(Segment_Sweep_In_Degrees) * i , _segmentOrigin, 1.0f, SpriteEffects.None, Segment_Render_Depth);
            }

            spriteBatch.Draw(TextureManager.Textures[Needle_Texture_Name], CenterPosition + _centerOffset, null, Color.White, NeedleAngle, _masterOrigin, 1.0f, 
                SpriteEffects.None, Needle_Render_Depth);

            spriteBatch.Draw(TextureManager.Textures[Center_Texture_Name], CenterPosition + _centerOffset, null, Color.White, 0.0f,
                new Vector2(TextureManager.Textures[Center_Texture_Name].Width, TextureManager.Textures[Center_Texture_Name].Height) / 2.0f, 1.0f,
                SpriteEffects.None, Center_Render_Depth);
