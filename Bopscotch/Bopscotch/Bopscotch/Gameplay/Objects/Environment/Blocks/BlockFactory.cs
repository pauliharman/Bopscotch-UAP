...
namespace Bopscotch.Gameplay.Objects.Environment.Blocks
{
    public sealed class BlockFactory
    {
        ...
        public static TileMap LoadBlocks(XElement blockDataGroup)
        {
            ...
            return Factory.CreateTileMap(worldDimensions, blocks);
        }
        ...
        private Block CreateBlock(XElement node)
        {
            ...
            newBlock.TextureReference = node.Attribute("texture").Value;
        ...
        private TileMap CreateTileMap(Point worldDimensions, List<Block> tiles)
        {
        ...
        public static void ReinstateSerializedBlocks(List<XElement> blockData)
        {
            foreach (XElement el in blockData)
            {
                Block newBlock = (Block)Activator.CreateInstance(Type.GetType(el.Attribute("type").Value));
                newBlock.ID = el.Attribute("id").Value;
            }
        }

        public const string Data_Group_Node_Name = "blocks";
        public const string Serialized_Data_Identifier = "block-";
