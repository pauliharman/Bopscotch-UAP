    ...
    public class GlowBurst
    {
        ...
        public void Draw(SpriteBatch spriteBatch)
        {
            if (Visible)
            {
                for (int i = 0; i < _components.Count; i++)
                {
                    spriteBatch.Draw(TextureManager.Textures[Texture_Name], Position, null, Tint, _components[i].Y,
                        new Vector2(TextureManager.Textures[Texture_Name].Width, TextureManager.Textures[Texture_Name].Height) / 2.0f,
                        (1.0f - (_scaleStep * i)) * _masterScale, SpriteEffects.None, RenderDepth + (Render_Depth_Step * i));
                }
            }
        }
